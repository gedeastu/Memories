package org.d3if3132.assesment03.memories.ui.presentation
...
class MainViewModel : ViewModel(){
    ...
    fun getData(userId: String){
        viewModelScope.launch(Dispatchers.IO){
            ...
            try {
                data.value = ItemApi.service.getItems(userId = userId)
    ...
    fun saveData(userId: String, title: String, description: String, bitmap: Bitmap){
        viewModelScope.launch(Dispatchers.IO){
            try {
                val result = ItemApi.service.postItem(
                    userId = userId,
                    title = title.toRequestBody("text/plain".toMediaTypeOrNull()),
                    description = description.toRequestBody("text/plain".toMediaTypeOrNull()),
                    imageId = bitmap.toMultipartBody()
                )
    ...
    fun deletingData(userId: String, id: String){
        viewModelScope.launch(Dispatchers.IO){
            try {
                val result = ItemApi.service.deleteItem(userId = userId, id = id)
                if (result.status == "success"){
                    getData(userId = userId)
                }else{
                    throw Exception(result.message)
